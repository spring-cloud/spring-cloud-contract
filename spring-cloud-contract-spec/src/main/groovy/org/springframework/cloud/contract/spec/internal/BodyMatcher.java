...
public interface BodyMatcher {
	...
	/**
	 * Optional value that the given path should be checked against. If there is no value
	 * then presence will be checked only together with type check. Example if we expect a
	 * JSON Path path {@code $.a} to be matched. by type, the defined response body
	 * contained an integer but the actual one contained a string then the assertion
	 * should fail.
	 */
	Object value();
