...
class RegexPatternsSpec extends Specification {
	...
	def "should generate a regex for url [#textToMatch] that is a match [#shouldMatch]"() {
		...
		where:
			...
			'http://www.foo.com/'                         || true
			'http://www.foo.com/(wikipedia)'              || true
			'http://www.foo.com/(wikipedia)_(again)'      || true
			...
			'http://⌘.ws'                                       || true
			'http://⌘.ws/'                                      || true
			'http://www.foo.com/(wikipedia)#cite-1'            || true
			'http://www.foo.com/(wikipedia)_blah#cite-1'       || true
			'http://www.foo.com/(✪)_in_parens'              || true
