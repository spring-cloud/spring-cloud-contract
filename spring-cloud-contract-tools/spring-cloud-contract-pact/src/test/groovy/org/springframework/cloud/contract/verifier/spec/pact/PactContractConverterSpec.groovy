...
'''
	...
	def "should work properly with cookies when reading a pact file as contract"() {
		...
		then:
			...
			generatedTestText.contains('''cookie("b", "2")''')
			generatedTestText.contains('''assertThat(response.getCookie("c")).isNotNull()''')
			generatedTestText.contains('''assertThat(response.getCookie("c")).isEqualTo("1")''')
			generatedTestText.contains('''assertThat(response.getCookie("d")).isNotNull()''')
			generatedTestText.contains('''assertThat(response.getCookie("d")).isEqualTo("2")''')
	}
