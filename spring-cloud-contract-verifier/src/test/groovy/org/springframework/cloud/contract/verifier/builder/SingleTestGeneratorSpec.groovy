...
''')
	...
	def "should build test class for #testFramework with Rest Assured 2x"() {
		...
		where:
			...
			TESTNG        | null  | MOCKMVC  | mockMvcTestNGRestAssured2ClassStrings
			TESTNG        | null  | EXPLICIT | explicitTestNGRestAssured2ClassStrings
...
''')
	...
	def 'should not allow the usage of ignore annotations for TestNG '() {
		given:
			TestFrameworkDefinition testNG = TESTNG

		when:
			boolean usesIgnoreAnnotations = testNG.usesIgnoreAnnotations()
		then:
			!usesIgnoreAnnotations
	...
	def 'should use ignore annotations for JUnit and Spock'() {
		expect:
			framework.usesIgnoreAnnotations() == true

		where:
			framework << [JUNIT, JUNIT5, SPOCK]
	}
