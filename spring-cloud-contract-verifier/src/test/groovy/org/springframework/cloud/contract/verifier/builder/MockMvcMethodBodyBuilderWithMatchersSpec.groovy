...
class MockMvcMethodBodyBuilderWithMatchersSpec extends Specification implements WireMockStubVerifier {
	...
	def 'should allow to set dynamic values via stub / test matchers for [#methodBuilderName]'() {
		...
		then:
			...
			test.contains('assertThat(parsedJson.read("' + rootElement + '.date", String.class)).matches("(\\\\d\\\\d\\\\d\\\\d)-(0[4,6,9]|11)-(0[1-9]|[12][0-9]|3[0])|(\\\\d\\\\d\\\\d\\\\d)-(0[1,3,5,7,8]|10|12)-(0[1-9]|[12][0-9]|3[01])|(\\\\d\\\\d\\\\d\\\\d)-(02)-(0[1-9]|[12][0-8])"')
			test.contains('assertThat(parsedJson.read("' + rootElement + '.dateTime", String.class)).matches("((\\\\d\\\\d\\\\d\\\\d)-(0[4,6,9]|11)-(0[1-9]|[12][0-9]|3[0])|(\\\\d\\\\d\\\\d\\\\d)-(0[1,3,5,7,8]|10|12)-(0[1-9]|[12][0-9]|3[01])|(\\\\d\\\\d\\\\d\\\\d)-(02)-(0[1-9]|[12][0-8]))T(2[0-3]|[01][0-9]):([0-5][0-9]):([0-5][0-9])"')
