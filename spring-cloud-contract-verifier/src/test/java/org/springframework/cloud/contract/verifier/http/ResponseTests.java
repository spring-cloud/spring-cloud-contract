...
class ResponseTests {
	...
	void should_override_entries_when_using_builder_from_response() {
		...
		then(changedResponse.headers()).containsEntry("header-foo", "header-bar")
			.containsEntry("header", "header-value");
		then(changedResponse.cookies()).containsEntry("cookie-foo", "cookie-bar")
			.containsEntry("cookie", "cookie-value");
	}
