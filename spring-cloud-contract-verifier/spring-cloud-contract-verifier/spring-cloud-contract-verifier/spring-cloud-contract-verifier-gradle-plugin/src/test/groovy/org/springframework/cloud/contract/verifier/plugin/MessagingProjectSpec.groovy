...
@Stepwise
class MessagingProjectSpec extends ContractVerifierIntegrationSpec {
	...
	def "should pass basic flow for Spock"() {
		...
		expect:
			...
			jarContainsContractVerifierContracts('build/libs')
	}
	...
	def "should pass basic flow for JUnit"() {
		given:
			...
			assert fileExists('build.gradle')
		expect:
			switchToJunitTestFramework('org.springframework.cloud.samples.book.MessagingBaseSpec', 'org.springframework.cloud.samples.book.MessagingBaseTest')
			runTasksSuccessfully('check', "publishToMavenLocal")
			jarContainsContractVerifierContracts('build/libs')
	}
